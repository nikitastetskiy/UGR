...
// Funci—n para calcular la distancia euclidea entre dos puntos (ciudades) [x1,y1]
// y [x2,y2] .
...
void Distancias (map<int, pair <double, double> > m, vector<vector<int> > & matriz){
    for (int i = 0; i < matriz.size(); i++)
        for (int j = 0; j < matriz[i].size(); j++)
            if (i != j)
                matriz[i][j] = DistanciaEuclidea(m[i+1].first, m[i+1].second, m[j+1].first, m[j+1]. second);
}
...
/* Tomamos como ciudad inicial la 1. Recorremos la fila 1 y seleccionamos la ciudad a menor distancia
   (columnas). Cuando hacemos esto saltamos a la fila de la ciudad seleccionada y volvemos a realizar
   este procedimiento. Tenemos encuenta que no podemos introducir ciudades repetidas. Cuando haya
   introducido todas las ciudades a la lista sin repetidciones, a–adimos a la distancia final el
   camino entre la ultima ciudad y la primera, e insertamos de nuevo la ciudad 1 (la œnica que se
   repite).
 */
vector<int> TSPAlg (vector< vector<int> > matriz, int & distancia){
    vector<int> recorrido;
    int ciudad, min, pos, j = 0;
